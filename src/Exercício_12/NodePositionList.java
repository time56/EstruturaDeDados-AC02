...
public class NodePositionList<E> implements PositionList<E> {
	...
	public void inverterLista() {
		int cont = 0;
		DNode<E> firstPos = (DNode<E>) this.first();
		DNode<E> lastPos = (DNode<E>) this.last();
		
		int comparator = (this.size() % 2 == 0) ? this.size() / 2 + 1 : this.size() / 2;
		while (comparator != cont) {
			E temp = this.set(firstPos, lastPos.element());
			this.set(lastPos, temp);
			firstPos = firstPos.getNext();
			lastPos = lastPos.getPrev();
			cont++;
		}
	}
